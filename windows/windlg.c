...
{
    WNDCLASSEX wc; // PuttyTray hack / Icon (changed from WNDCLASS to WNDCLASSEX)
    HWND hwnd;
    MSG msg;
    int flags;
    int ret;
    ...
    wc.hInstance = hinst;
    /* PuttyTray hack / Icon (2 lines, plus one commented line) */
 	 wc.hIcon = LoadImage(hinst, MAKEINTRESOURCE(IDI_CFGICON), IMAGE_ICON, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR|LR_SHARED);
 	 wc.hIconSm = LoadImage(hinst, MAKEINTRESOURCE(IDI_CFGICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR|LR_SHARED);
//    wc.hIcon = NULL;
    wc.hCursor = LoadCursor(NULL, IDC_ARROW);
    wc.hbrBackground = (HBRUSH) (COLOR_BACKGROUND +1);
    wc.lpszMenuName = NULL;
    wc.lpszClassName = "PuTTYConfigBox";
    RegisterClassEx(&wc); /* PuttyTray hack / Icon ... changed RegisterClass to RegisterClassEx */
...
static void create_controls(HWND hwnd, char *path)
{
	 ...
	 */
	ctlposinit(&cp, hwnd, 3, 3, 285); /* PuttyTray hack / changed size from 235 to 285 */
	wc = &ctrls_base;
				       ...
				       WPARAM wParam, LPARAM lParam)
{
   ...
   /* PuttyTray hack / Comment the next 2 lines */
	 ...
	 */
	{
	    ...
	    WPARAM font;
	    HWND tvstatic;
	    /* PuttyTray hack / System font (2 lines) */
	    ...
	    /* PuttyTray hack / System font (9 lines, 2 commented lines) */
		 ...
		 SendMessage(tvstatic, WM_SETFONT, (WPARAM)hDlgFont, MAKELPARAM(TRUE, 0));
	    ...
	    r.right = r.left + 95;
	    r.top = 13;
	    r.bottom = r.top + 269; /* PuttyTray hack / Changed from 219 to 269 */
	    MapDialogRect(hwnd, &r);
	    treeview = CreateWindowEx(WS_EX_CLIENTEDGE, WC_TREEVIEW, "",
				      ...
				      NULL);
	    /* PuttyTray hack / System font (1 line, 2 comment lines) */
	    SendMessage(treeview, WM_SETFONT, (WPARAM)hDlgFont, MAKELPARAM(TRUE, 0));
	    //font = SendMessage(hwnd, WM_GETFONT, 0, 0);
