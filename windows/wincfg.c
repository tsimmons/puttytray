...
/* PuttyTray hack / Transparency ... for is_glass_available() (1 line) */
#include "winstuff.h"
...
/* PuttyTray hack / Session icon function (22 lines) */
			  ...
			  int midsession, int protocol)
{
    ...
    union control *c;
    char *str;
	/* PuttyTray hack / Transparency ... added option (1 line) */
	BOOL with_glass;
    ...
    /* PuttyTray hack / Transaparency, Reconnect (19 lines) */
    ...
    ctrl_checkbox(s, "System menu appears on ALT-Space", 'm', /* PuttyTray hack / Changed shortcut from 'y' to 'm' */
    ...
    ctrl_checkbox(s, "Full screen on Alt-Enter", 'f',
 	...
 	/* PuttyTray hack / Settings dialog (70 lines) */
    ctrl_radiobuttons(s, "Show tray icon:", NO_SHORTCUT, 4,
     ...
     ctrl_checkbox(s, "Accept single-click to restore from tray", 'p',
 		  HELPCTX(no_help),
 		  conf_checkbox_handler,
 		  I(CONF_tray_restore));
     s = ctrl_getset(b, "Window/Behaviour", "icon", "Other adjustments");
     ...
     ctrl_columns(s, 1, 100);
 	  s = ctrl_getset(b, "Window/Behaviour", "icon", "Window position");
     ctrl_columns(s, 3, 25, 25, 50);
     ctrl_checkbox(s, "Use fixed startup position", NO_SHORTCUT,
		 HELPCTX(appearance_hidemouse),
		 conf_checkbox_handler,
		 I(CONF_save_windowpos));
     c = ctrl_editbox(s, "Top:", NO_SHORTCUT, 50,
		 HELPCTX(appearance_border),
		 conf_editbox_handler,
		 I(CONF_windowpos_top), I(-1));
     c->generic.column = 0;
     c = ctrl_editbox(s, "Left:", NO_SHORTCUT, 50,
		 HELPCTX(appearance_border),
		 conf_editbox_handler,
		 I(CONF_windowpos_left), I(-1));
     c->generic.column = 1;
     ctrl_columns(s, 1, 100);
