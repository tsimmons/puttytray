...
/* PuttyTray hack / Hyperlinks (1 line) */
#include "urlhack.h"
...
/* PuttyTray hack / Flash TrayIcon (1 line) */
static void flash_icon(int mode);
...
/* PuttyTray hack / TrayIcon struct, global booleans, icon, functions (8 lines) */
static NOTIFYICONDATA puttyTray;
static BOOL puttyTrayIconVisible;
static BOOL puttyTrayFlash;
static BOOL windowMinimized = FALSE;
static HICON puttyTrayIcon;
...
/* PuttyTray hack / Always on top (12 lines) */ 
...
/* PuttyTray hack / Transparency (3 lines) */
...
/* PuttyTray hack / Reconnect (1 line) */
static time_t last_reconnect = 0;

/* PuttyTray hack / Transparency (4 functions, 48 lines) */
...
{
	// We need to only consider valid values for transparency, because some other transparency hacks use different scales!
	// We only honor values of 0, 1, 2, or 3
	if(conf_get_int(conf, CONF_transparency) <= 3) {
		 BOOL opaque = conf_get_int(conf, CONF_opaque_when_focused) && term->has_focus;
		 if (pSetLayeredWindowAttributes) {
			  int trans = max(conf_get_int(conf, CONF_transparency), 0);
		 ...
		 enable_glass(conf_get_int(conf, CONF_transparency) < 0 && !is_full_screen() && !opaque);
	}
}
/* PuttyTray hack / end */
...
{
    WNDCLASSEX wndclass; /* PuttyTray hack / changed WNDCLASS to WNDCLASSEX */
    MSG msg;
    HRESULT hr;
    int guess_width, guess_height;
    /* PuttyTray hack / Remember Window Position @ http://www.bradgoodman.com/puttypatch.html (1 line) */
    int use_left, use_top;
    ...
    sk_init();
	 ...
	 /* PuttyTray hack / Transparency (1 line) */
    load_funcs();
    ...
    if (!prev) {
    /* PuttyTray hack / Session icon (1 line) */
	...
	/* PuttyTray hack / Added some additions for two icon sizes (9 lines, 1 commented line) */
	if (conf_get_str(conf, CONF_win_icon)[0]) {
		...
		// Not sure why, but if the instance parameter is NOT "NULL", this fails to set the small window icon
		wndclass.hIconSm = (HICON) LoadImage(NULL, MAKEINTRESOURCE(IDI_TRAYICON), IMAGE_ICON, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR|LR_SHARED);
	}
	...
	RegisterClassEx(&wndclass); /* PuttyTray hack / Changed from RegisterClass to RegisterClassEx */
    }
	...
	/* PuttyTray hack / Remember Window Position (7 lines) */
	if (conf_get_int(conf, CONF_save_windowpos)) {
		use_left = conf_get_int(conf, CONF_windowpos_left);
		use_top = conf_get_int(conf, CONF_windowpos_top);
	} else {
	...
	hwnd = CreateWindowEx(exwinmode, appname, appname,
			      winmode, use_left, use_top, 			 /* PuttyTray hack / Remember Window Position (added this line) */
//			      winmode, CW_USEDEFAULT, CW_USEDEFAULT, /* PuttyTray hack / Remember Window Position (commented this line) */
	...
	for (j = 0; j < lenof(popup_menus); j++) {
		...
		/* PuttyTray hack / Always on top (5 lines) */
		if (conf_get_int(conf, CONF_alwaysontop)) {
	...
	/* PuttyTray hack / TrayIcon (10 lines) */
	...
	if (conf_get_str(conf, CONF_win_icon)[0]) {
		...
		puttyTray.hIcon	= (HICON) LoadImage(inst, MAKEINTRESOURCE(IDI_TRAYICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR|LR_SHARED);
	}

	/* PuttyTray hack / Menu properties (8 lines) */
	{
		MENUINFO mi;
	...
	/* PuttyTray hack / Hyperlinks ... Set the regular expression (3 lines) */
	...
	/* PuttyTray hack / Finally show the window (or the trayicon)! (13 lines, 2 commented lines) */
	puttyTrayIconVisible = FALSE;	
...
{
	 /* PuttyTray hack / Remove trayicon on close (2 lines) */
...
/* PuttyTray / Reconnect ... output a string to the terminal window ... (3 lines) */
void c_write_str(void *frtend, char *str) {
	from_backend(frtend,0,str,strlen(str));
}
...
void connection_fatal(void *frontend, char *fmt, ...)
{
    ...
    char *stuff, morestuff[100];
	 ...
	 /* PuttyTray / Reconnect ... comment out this block of code, replace it with the following (12 lines commented, 27 lines added) */
    //va_start(ap, fmt);
...
#define f(i,c,w,u) \
	...
	/* PuttyTray hack / ClearType font bold fix -- this block of code is moved from below ... See http://stackoverflow.com/questions/2520610/detecting-cleartype-optimized-fonts (3 lines) */ 
				...
				WPARAM wParam, LPARAM lParam)
{
	...
	switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
	  ...
	  case IDM_RECONF:
		...
		/* PuttyTray hack / Session Icon (2 lines) */
		HINSTANCE inst;
		HICON hIcon;
		...
		/* PuttyTray hack / Hyperlinks ... Reconfigure (5 lines) */
		if (conf_get_int(conf, CONF_url_defregex) == 0) {
			urlhack_set_regular_expression(conf_get_str(conf, CONF_url_regex));
		}
		...
		/* PuttyTray / Session Icon ... Reconfigure (16 lines) */
		if (conf_get_str(conf, CONF_win_icon)[0]) {
			hIcon = extract_icon(conf_get_str(conf, CONF_win_icon), TRUE);
			DestroyIcon(puttyTray.hIcon);
			puttyTray.hIcon = hIcon;
			SetClassLong(hwnd, GCL_HICON, (LONG)extract_icon(conf_get_str(conf, CONF_win_icon), FALSE));
			SetClassLong(hwnd, GCL_HICONSM, (LONG)hIcon);
		} else {
			inst = (HINSTANCE)GetWindowLong(hwnd, -6);
			DestroyIcon(puttyTray.hIcon);
			puttyTray.hIcon	= (HICON) LoadImage(inst, MAKEINTRESOURCE(IDI_TRAYICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR|LR_SHARED);
			...
			SetClassLong(hwnd, GCL_HICONSM, (LONG)LoadImage(inst, MAKEINTRESOURCE(IDI_TRAYICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTSIZE|LR_SHARED));
		}
		if (puttyTrayIconVisible) {
			taskbar_addicon(conf_get_int(conf, CONF_win_name_always) ? window_name : icon_name, TRUE);
		}

		/* PuttyTray / TrayIcon Reconfigure (12 lines) */
		...
		/* Enable or disable the scroll bar, etc */
		{
			...
			if (conf_get_int(conf, CONF_alwaysontop)) {
			    ...
			    SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0,
				 ...
				 /* PuttyTray hack / Always on top menu (1 line) */
	  ...
	  /* PuttyTray hack / Always on top case (3 lines) */
	  case IDM_VISIBLE: 
	  	MakeWindowOnTop(hwnd);
		break ;

	  /* PuttyTray / TrayIcon Menu addon click handlers (13 lines) */
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
	...
	/* PuttyTray hack / Hyperlinks ... Change cursor type if hovering over link (23 lines) */
	if (urlhack_mouse_old_x != TO_CHR_X(X_POS(lParam)) || urlhack_mouse_old_y != TO_CHR_Y(Y_POS(lParam))) {
		...
		if ((!conf_get_int(conf, CONF_url_ctrl_click) || urlhack_is_ctrl_pressed()) &&
	...
	/* PuttyTray hack / Addon to SIZE_MINIMIZED for adding/removing the trayicon (18 lines, 4 commented lines) */
...
//			  window_name : icon_name);
	...
	return FALSE;
      case WM_KEYDOWN:
      	/* PuttyTray hack / Hyperlinks (10 lines) */
		if (wParam == VK_CONTROL && conf_get_int(conf, CONF_url_ctrl_click)) {
      ...
      case WM_SYSKEYDOWN:
      case WM_KEYUP:
      	/* PuttyTray hack / Hyperlinks (5 lines) */
		if (wParam == VK_CONTROL && conf_get_int(conf, CONF_url_ctrl_click)) {
			SetCursor(LoadCursor(NULL, IDC_IBEAM));
			term_update(term);
			goto KEY_END;
		}
		/* PuttyTray hack / Hyperlinks goto placemarker (1 line) */
		KEY_END:
      case WM_SYSKEYUP:
	/*
	...
	/* PuttyTray hack / TrayIcon handler (37 lines) */
	...
	/* PuttyTray hack / Reconnect handler (28 lines) */
...
void set_title(void *frontend, char *title)
{
	...
	/* PuttyTray hack / Change Trayicon Tooltip to window title (1 line) */
	taskbar_addicon(conf_get_int(conf, CONF_win_name_always) ? window_name : icon_name, puttyTrayIconVisible);
}
...
/* PuttyTray hack / Flash TrayIcon (7 lines) */
static void flash_icon(int mode);
static long next_flash_icon = 0;
static void flash_icon_timer(void *ctx, unsigned long now) {
    if (flashing && now == next_flash_icon) {
        flash_icon(1);
    }
}
...
static void flash_window(int mode)
{
    ...
    if (flashing) {
        flashing = 0;
        /* PuttyTray hack / Stop TrayIcon flashing (1 line) */
        flash_icon(0);
        if (p_FlashWindowEx)
    ...
    if (!flashing) {
        flashing = 1;
        /* PuttyTray hack / Start TrayIcon flashing (1 line) */
        next_flash_icon = schedule_timer(400, flash_icon_timer, hwnd);
        if (p_FlashWindowEx) {
...
/* PuttyTray hack / TrayIcon flash (1 function, 35 lines) */
static void flash_icon(int mode) {
    if(mode == 1) {
        if (puttyTrayIconVisible) {
            if (!puttyTrayFlash) {
                puttyTrayFlash = TRUE;
                puttyTrayIcon = puttyTray.hIcon;
                puttyTray.hIcon = NULL;
                taskbar_addicon(conf_get_int(conf, CONF_win_name_always) ? window_name : icon_name, TRUE);
            } else {
                puttyTrayFlash = FALSE;
                puttyTray.hIcon = puttyTrayIcon;
                taskbar_addicon(conf_get_int(conf, CONF_win_name_always) ? window_name : icon_name, TRUE);
            }
        } else {
            if (puttyTray.hIcon == NULL) {
                puttyTray.hIcon = puttyTrayIcon;
            }
        }
        next_flash_icon = schedule_timer(400, flash_icon_timer, hwnd);
...
void set_iconic(void *frontend, int iconic)
{
	...
	if (!iconic) {
	    ShowWindow(hwnd, SW_RESTORE);
	    /* PuttyTray hack / Also added braces to if() statement (1 line) */
		 windowMinimized = FALSE;
	}
    } else {
	if (iconic) {
...
/* PuttyTray hack / Everything below here is new */
/* PuttyTray hack / Function to add icon to the taskbar's system tray */
BOOL taskbar_addicon(LPSTR lpszTip, BOOL showIcon) 
{ 
    BOOL icon_result; 
...
/* PuttyTray hack / Function to update tray icon */
void tray_updatemenu(BOOL disableMenuItems)
{
	MENUITEMINFO mii;
...
/* PuttyTray hack / Always on top / Function to switch the window positioning to and from 'always on top' */ 
void MakeWindowOnTop(HWND hwnd) {
	HMENU m;
