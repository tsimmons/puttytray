...
static void do_paint(Terminal *term, Context ctx, int may_optimise)
{
	...
	/* PuttyTray hack / Hyperlinks ... TODO: We should find out somehow that the stuff on screen has changed since last paint. How to do it? (28 lines) */
	...
	if (term->url_update) {
		urlhack_reset();
		for (i = 0; i < term->rows; i++) {
			// Toby says this is where the scroll crash is happening ... changed from scrlineptr to lineptr. Not sure what the diff really is
//			termline *lp = scrlineptr(term->disptop + i);
	...
	for (j = 0; j < term->cols; j++) {
		...
		tattr |= ATTR_WIDE;
 		...
 		/* PuttyTray hack / Hyperlinks -- Underline link regions if user has configured us so (28 lines) */
		...
		Mouse_Action a, int x, int y, int shift, int ctrl, int alt)
{
	...
	term->selmode = SM_CHAR;
 	/* PuttyTray hack / Hyperlinks ... handle click inside link region, if so -> copy url to temporary buffer and launch it. Delete the temporary buffer. (34 lines) */
 	...
 	if ((!conf_get_int(term->conf, CONF_url_ctrl_click) || (conf_get_int(term->conf, CONF_url_ctrl_click) && urlhack_is_ctrl_pressed())) && urlhack_is_in_link_region(x, y)) {
 		...
 		urlhack_launch_url(!conf_get_int(term->conf, CONF_url_defbrowser) ? (char *) conf_get_filename(term->conf, CONF_url_browser) : NULL, linkbuf);
 		sfree(linkbuf);
 	}
 	/* PuttyTray hack / Hyperlinks ... end */
